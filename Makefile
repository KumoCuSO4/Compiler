...
JAVAC = javac -g -cp /usr/local/lib/antlr-4.9.2-complete.jar:.
JAVA = java -cp /usr/local/lib/antlr-4.9.2-complete.jar:.
...
TESTS = $(shell find ./Tests_advanced/tests -name "*.cmm")
EXPECTS = $(shell find ./Tests_advanced/expects -name "*.output")
SHELL=/bin/bash
...
run: compile
	java -classpath ./classes:$(ANTLRPATH) Main $(FILEPATH) $(OUTPATH) 
